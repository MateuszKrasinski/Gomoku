...
class Button:
    def __init__(self, next=0, name="Button"):
        self.border = pygame.Rect(screenWidth - rightMargin + rightMargin // 20 - 2, screenHeight // 3 + (next * (squareWidth + 2 * squareMargin)) - 2, rightMargin * 9 // 10 + 4,
                                  4 // 4 * squareWidth + 4)
        self.graphic = pygame.Rect(screenWidth - rightMargin + rightMargin // 20, screenHeight // 3 + (next * (squareWidth + 2 * squareMargin)), rightMargin * 9 // 10,
                                   4 // 3 * squareWidth)
...
class ChooseOpponent():
    def __init__(self, number):
        self.number = number
        self.graphic = pygame.Rect(screenWidth / 2 + number * 50 - 25, 10, 60, 30)
        self.border = pygame.Rect(screenWidth / 2 + number * 50 - 25 - 2, 10 - 2, 60 + 4, 30 + 4)
...
class ChooseMode():
    def __init__(self, number):
        ...
        self.border = pygame.Rect(boardWidth - 30 + number * 70 - 2, 10 - 2, 70 + 4, 30 + 4)
