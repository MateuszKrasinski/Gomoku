            ...
            for j in range(0, constants.BOARD_SIZE - i - 1):
                if self.game_board[j + i][j] == self.game_board[j + i + 1][
                    ...
                    if consecutive == WIN:
                        return True
                    if consecutive == 2:
                        if self.game_board[j + i + 1][j + 1] == constants.EMPTY:
                            if self.game_board[j + i - 2][j - 2] == constants.EMPTY:
                        ...
                        if self.game_board[j + i + 1][j + 1] == constants.EMPTY:
                            if self.game_board[j + i - 3][j - 3] == constants.EMPTY:
                                if self.game_board[j + i][j] == constants.WHITE:
                                    self.three_stones += 1
                                else:
                                    self.three_stones -= 1
                        ...
                        if self.game_board[j + i + 1][j + 1] == constants.EMPTY:
                            if self.game_board[j + i - 4][j - 4] == constants.EMPTY:
                                if self.game_board[j + i][j] == constants.WHITE:
                                    self.four_stones += 1
                                else:
                                    self.four_stones -= 1
            ...
            for j in range(0, constants.BOARD_SIZE - i - 1):
                if self.game_board[j][j + i] == self.game_board[j + 1][
                    ...
                    if consecutive == WIN:
                        return True
                    if consecutive == 2:
                        if self.game_board[j + 1][j + i + 1] == constants.EMPTY:
                            if self.game_board[j - 2][j + i - 2] == constants.EMPTY:
                        ...
                        if self.game_board[j + 1][j + i + 1] == constants.EMPTY:
                            if self.game_board[j - 3][j + i - 3] == constants.EMPTY:
                                if self.game_board[j][j + i] == constants.WHITE:
                                    self.three_stones += 1
                                else:
                                    self.three_stones -= 1
                        ...
                        if self.game_board[j + 1][j + i + 1] == constants.EMPTY:
                            if self.game_board[j - 4][j + i - 4] == constants.EMPTY:
                                if self.game_board[j][j + i] == constants.WHITE:
                                    self.four_stones += 1
                                else:
                                    self.four_stones -= 1
            ...
            for j in range(0, i):
                if self.game_board[i - j][j] == self.game_board[i - j - 1][
                    ...
                    if consecutive == 2 and i - j + 2 <= constants.BOARD_SIZE - 1:
                        if self.game_board[i - j + 2][j - 2] == constants.EMPTY:
                            if self.game_board[i - j - 1][j + 1] == constants.EMPTY:
                                if self.game_board[i - j][j] == constants.WHITE:
                                    self.two_stones += 1
                                else:
                                    self.two_stones -= 1
                    elif consecutive == 3 and i - j + 3 <= constants.BOARD_SIZE - 1:
                        if self.game_board[i - j + 3][j - 3] == constants.EMPTY:
                            if self.game_board[i - j - 1][j + 1] == constants.EMPTY:
                                if self.game_board[i - j][j] == constants.WHITE:
                                    self.three_stones += 1
                                else:
                        ...
                        if self.game_board[i - j + 4][j - 4] == constants.EMPTY:
                            if self.game_board[i - j - 1][j + 1] == constants.EMPTY:
                                if self.game_board[i - j][j] == constants.WHITE:
                                    self.four_stones += 1
                                else:
...
def create_board():
    """Creating empty board 15x15."""
    return [[constants.EMPTY for i in range(constants.BOARD_SIZE)] for j in range(
